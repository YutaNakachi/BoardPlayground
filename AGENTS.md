...
- プレイ中に出たり消えたりする短いメッセージ（パス通知、非法手、フェーズ案内など）は、基本 **TurnBanner 内の1行**（`action` または `notice`）に表示する。
...
- `stats` / `action` / `notice` は行の出し入れで盤が動かないよう、空でも高さを確保する（`components/play/shared/TurnBanner.tsx`）。
- `action` は次にやること、`notice` は状況通知（パス・非法手など）。
